/*
---------------------------------------------------------------------------
 $Id: tsp_redirect.c,v 1.4 2007/11/28 17:27:36 cnepveu Exp $
---------------------------------------------------------------------------
...
int tspIsRedirectStatus(int status) {
...
tRedirectStatus tspAddBrokerToList(tBrokerList **broker_list, char *address, tBrokerAddressType address_type, unsigned int distance) {
	tBrokerList *new_broker = NULL;

	/* Some internal error checking */
	if (broker_list == NULL) {
		Display(LOG_LEVEL_3, ELError, "tspAddBrokerToList", HEX_STR_RDR_ADD_BROKER_INTERNAL_ERR);
	...
	if ((new_broker = (tBrokerList *)malloc(sizeof(tBrokerList))) == NULL) {
		Display(LOG_LEVEL_3, ELError, "tspAddBrokerToList", HEX_STR_RDR_ADD_BROKER_NO_MEM);
	...
	if (strlen(new_broker->address) != strlen(address)) {
		free(new_broker);
		Display(LOG_LEVEL_3, ELError, "tspAddBrokerToList", HEX_STR_RDR_ADD_BROKER_ADDRESS_TRUNC);
...
tRedirectStatus tspLogRedirectionList(tBrokerList *broker_list, int sorted) {
	...
	if (sorted) {
		Display(LOG_LEVEL_1, ELInfo, "tspLogRedirection", HEX_STR_RDR_SORTED_BROKER_LIST_IS, broker_list_string);
	}
	else {
		Display(LOG_LEVEL_1, ELInfo, "tspLogRedirection", HEX_STR_RDR_BROKER_LIST_IS, broker_list_string);
	}
...
tRedirectStatus tspCreateBrokerList(tTunnel *tunnel_info, tBrokerList **broker_list, int *broker_count) {
	...
	for (current_broker = tunnel_info->broker_redirect_ipv4; current_broker != NULL; current_broker = current_broker->next) {
		if (*broker_count < MAX_REDIRECT_BROKERS_IN_LIST) { 
			...
			if (tspAddBrokerToList(broker_list, current_broker->Value, TSP_REDIRECT_BROKER_TYPE_IPV4, 0) != TSP_REDIRECT_OK) {
				Display(LOG_LEVEL_3, ELError, "tspCreateBrokerList", HEX_STR_RDR_CREATE_LIST_CANT_ADD);
				tspFreeBrokerList(*broker_list);
				*broker_list = NULL;
			...
			Display(LOG_LEVEL_2, ELError, "tspCreateBrokerList", HEX_STR_RDR_TOO_MANY_BROKERS, MAX_REDIRECT_BROKERS_IN_LIST);
			tspFreeBrokerList(*broker_list);
			*broker_list = NULL;
...
tRedirectStatus tspSortBrokerList(tBrokerList **broker_list, tConf *conf, int broker_count) {
	tBrokerList *sorted_list = NULL;
	tBrokerList *current_broker = NULL;
	tBrokerList *new_element = NULL;

	Display(LOG_LEVEL_2, ELInfo, "tspSortBrokerList", HEX_STR_RDR_SORTING_BROKER_LIST);

	/* Get the distance values */
	if (tspGetBrokerDistances(*broker_list, broker_count, conf) != TSP_REDIRECT_OK) {
		Display(LOG_LEVEL_3, ELError, "tspSortBrokerList", HEX_STR_RDR_SORT_LIST_CANT_GET_DIST);
	...
	while (current_broker != NULL) {
		...
		if ((new_element = (tBrokerList *)malloc(sizeof(tBrokerList))) == NULL) {
			tspFreeBrokerList(sorted_list);
			Display(LOG_LEVEL_3, ELError, "tspSortBrokerList", HEX_STR_RDR_SORT_LIST_CANT_ALLOC);
			return TSP_REDIRECT_CANT_ALLOCATE_MEM;
		}	
    ...
    new_element->address_type = current_broker->address_type;
		...
		if (tspInsertInBrokerList(new_element, &sorted_list) != TSP_REDIRECT_OK) {
			tspFreeBrokerList(sorted_list);
			Display(LOG_LEVEL_3, ELError, "tspSortBrokerList", HEX_STR_RDR_SORT_LIST_CANT_INSERT);
...
tRedirectStatus tspLogReceivedRedirection(char *payload, tConf *conf) {
	...
	Display(LOG_LEVEL_1, ELInfo, "tspLogReceivedRedirection", HEX_STR_RECEIVED_REDIRECTION, conf->server, status); 
...
tRedirectStatus tspHandleRedirect(char *payload, tConf *conf, tBrokerList **broker_list) {
	...
	if (tspExtractPayload(payload, &tunnel_info) != 0) {
		Display(LOG_LEVEL_2, ELError, "tspHandleRedirect", HEX_STR_RDR_CANT_EXTRACT_PAYLOAD);
	...
	if (tspCreateBrokerList(&tunnel_info, broker_list, &broker_count) != TSP_REDIRECT_OK) {
		Display(LOG_LEVEL_2, ELError, "tspHandleRedirect", HEX_STR_RDR_CANT_CREATE_LIST);
	...
	if (tspLogRedirectionList(*broker_list, 0) != TSP_REDIRECT_OK) {
		Display(LOG_LEVEL_2, ELError, "tspHandleRedirect", HEX_STR_RDR_CANT_LOG);
	...
	if (broker_count > 1) {
		...
		if (tspSortBrokerList(broker_list, conf, broker_count) != TSP_REDIRECT_OK) {
			Display(LOG_LEVEL_2, ELError, "tspHandleRedirect", HEX_STR_RDR_CANT_SORT_LIST);
		...
		if (tspWriteBrokerListToFile(conf->broker_list, *broker_list) != TSP_REDIRECT_OK) {
			Display(LOG_LEVEL_2, ELError, "tspHandleRedirect", HEX_STR_RDR_CANT_SAVE_LIST);
	...
	if (broker_count == 0) {
		Display(LOG_LEVEL_2, ELError, "tspHandleRedirect", HEX_STR_RDR_NULL_LIST);
...
tRedirectStatus tspReadLastServerFromFile(char *last_server_file, char *buffer) {
	...
	if (last_server_file == NULL) {
		Display(LOG_LEVEL_3, ELError, "tspReadLastServerFromFile", HEX_STR_RDR_READ_LAST_SERVER_INT_ERR);
		return TSP_REDIRECT_INTERNAL_ERR;
	}
...
tRedirectStatus tspWriteLastServerToFile(char *last_server_file, char *last_server) {
	...
	/* Some internal checking */
	if (last_server_file == NULL) {
		Display(LOG_LEVEL_3, ELError, "tspWriteLastServerToFile", HEX_STR_RDR_WRITE_LAST_SERVER_INT_ERR);
		return TSP_REDIRECT_INTERNAL_ERR;
	}
	...
	if ((file = fopen(last_server_file, "w")) == NULL) {
		Display(LOG_LEVEL_3, ELError, "tspWriteLastServerToFile", HEX_STR_RDR_WRITE_LAST_SERVER_CANT_OPEN, last_server_file);
		return TSP_REDIRECT_CANT_OPEN_FILE;
	}

	/* Write the last server to the last_server file */
	if (fprintf(file, "%s\n", last_server) < 0) {
		Display(LOG_LEVEL_3, ELError, "tspWriteLastServerToFile", HEX_STR_RDR_WRITE_LAST_SERVER_CANT_WRITE, last_server, last_server_file);
		fclose(file);
...
tRedirectStatus tspWriteBrokerListToFile(char *broker_list_file, tBrokerList *broker_list) {
	...
	if (broker_list_file == NULL) {
		Display(LOG_LEVEL_3, ELError, "tspWriteBrokerListToFile", HEX_STR_RDR_WRITE_BROKER_LIST_INT_ERR);
		return TSP_REDIRECT_INTERNAL_ERR;
	}

	/* Try to open the broker_list file */
	if ((file = fopen(broker_list_file, "w")) == NULL) {
		Display(LOG_LEVEL_3, ELError, "tspWriteBrokerListToFile", HEX_STR_RDR_WRITE_BROKER_LIST_CANT_OPEN, broker_list_file);
		return TSP_REDIRECT_CANT_OPEN_FILE;
	}
	...
	for (current_broker = broker_list; current_broker != NULL; current_broker = current_broker->next) {
		...
		if (fprintf(file, "%s\n", current_broker->address) < 0) {
			Display(LOG_LEVEL_3, ELError, "tspWriteBrokerListToFile", HEX_STR_RDR_WRITE_BROKER_LIST_CANT_WRITE, broker_list_file);
			fclose(file);
			return TSP_REDIRECT_CANT_WRITE_TO_FILE;
		}
	}
...
tRedirectStatus tspReadBrokerListFromFile(char *broker_list_file, tBrokerList **broker_list) {
	...
	/* Some internal checking */
	if (broker_list_file == NULL) {
		Display(LOG_LEVEL_3, ELError, "tspReadBrokerListFromFile", HEX_STR_RDR_WRITE_BROKER_LIST_INT_ERR);
		return TSP_REDIRECT_INTERNAL_ERR;
	}
		...
		if (strlen(line) > 0) {
			...
			if (broker_count < MAX_REDIRECT_BROKERS_IN_LIST) {
				...
				if (tspAddBrokerToList(broker_list, line, TSP_REDIRECT_BROKER_TYPE_NONE, 0) != TSP_REDIRECT_OK) {
					Display(LOG_LEVEL_3, ELError, "tspReadBrokerListFromFile", HEX_STR_RDR_READ_BROKER_LIST_CANT_ADD, broker_list_file);
					fclose(file);
					return TSP_REDIRECT_CANT_ADD_BROKER_TO_LIST;
				}
				else {
