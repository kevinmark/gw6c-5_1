/*
---------------------------------------------------------------------------
 $Id: tsp_auth_passdss.c,v 1.16 2007/11/28 17:27:36 cnepveu Exp $
---------------------------------------------------------------------------
...
#define TSPC_DSA_KEYFILE "gw6ckeys.pub"
...
is_dsakey_in_keyfile(DSA *dsa_1, char *host, char *filename) 
{
  ...
  if ( (line_buf = malloc(sizeof(char) * 4096)) == NULL) {
    ret = 0;
    goto is_dsakey_in_keyfile_cleanup;
  }
  ...
  while (freadline(line_buf, 4095, fp) != EOF) {
    ...
    */
    ...
    str_size = strlen(str_base64) * sizeof(char);
 ...
 */
int
add_dsakey_to_keyfile(DSA *dsa, char *host, char *filename) 
{
  ...
  case 0:
    Display(LOG_LEVEL_3, ELInfo, TSP_AUTH_PASSDSS_STRING, HEX_STR_ERR_IN_KEY_VERIF);
	  Display(LOG_LEVEL_3, ELWarning, TSP_AUTH_PASSDSS_STRING, HEX_STR_SERVER_KEY_REJECTED);
    break;
...
//
    if (!IsService) {
#endif
    if (!ask(HEX_STR_UNKNOWN_HOST_ADD_KEY, host)) {
		Display(LOG_LEVEL_3, ELWarning, TSP_AUTH_PASSDSS_STRING, HEX_STR_SERVER_KEY_REJECTED_USER);
        break;
	}
	...
	Display(LOG_LEVEL_1, ELWarning, TSP_AUTH_PASSDSS_STRING, HEX_STR_WARN_SERVER_KEY_AUTO_ADDED);
#endif

    Display(LOG_LEVEL_2, ELInfo, TSP_AUTH_PASSDSS_STRING, HEX_STR_SERVER_KEY_ACCEPTED_ADDED);
    ...
    if ( (str = malloc(2 * buffer_len(&buf))) == NULL)
      break;
  ...
  case 2: /* in and matching correctly, hurray */
    Display(LOG_LEVEL_2, ELInfo, TSP_AUTH_PASSDSS_STRING, HEX_STR_MATCHING_KEY_FOUND_USED);
    ret = 1;
    break;
  case 3: /* in and NOT matching correctly */
    Display(LOG_LEVEL_1, ELError, TSP_AUTH_PASSDSS_STRING, HEX_STR_WARN_STORED_LOCAL_KEY_NO_MATCH, filename, host);
	Display(LOG_LEVEL_3, ELWarning, TSP_AUTH_PASSDSS_STRING, HEX_STR_SERVER_KEY_REJECTED);
    ret = 0;
    break;
  }
 ...
 * 
 */
int AuthPASSDSS_3DES_1(SOCKET socket, net_tools_t *nt, tConf *conf, tBrokerList **broker_list)
{
 ...
 */
	...
	if ((dh = DH_new()) == NULL || (server_pubkey = BN_new()) == NULL) {
		Display(LOG_LEVEL_3, ELError, TSP_AUTH_PASSDSS_STRING, HEX_STR_MALLOC_ERROR);
		ret = -1;
		goto error;
	}
	...
	if ((dh->p == NULL) || (dh->g == NULL)) {
		Display(LOG_LEVEL_3, ELError, TSP_AUTH_PASSDSS_STRING, HEX_STR_INITIALIZATION_ERROR);
		ret = -1;
		goto error;
	}
	if ((dh_K = BN_new()) == NULL) {
	...
	*/
	if (DH_generate_key(dh) == 0) {
		Display(LOG_LEVEL_3, ELError, TSP_AUTH_PASSDSS_STRING, HEX_STR_DH_GEN_ERROR);
		ret = -1;
		goto error;
	}
	...
	if ( nt->netsend(socket, authenticate, sizeof(authenticate)) == -1 ) {
		Display(LOG_LEVEL_3, ELError, TSP_AUTH_PASSDSS_STRING,
			HEX_STR_CANT_W_SERVER_SOCKET);
		ret = -1;
		goto error;
	}
	...
	*/
	 ...
	 */
	if (pklength < 240) {
		Display(LOG_LEVEL_3, ELError, TSP_AUTH_PASSDSS_STRING, HEX_STR_RCVD_DATA_INVALID);
		ret = -1;
		goto error;
	}
	...
	{
	  if (!add_dsakey_to_keyfile(dsa, conf->server, TSPC_DSA_KEYFILE)) {
	    Display(LOG_LEVEL_3, ELError, TSP_AUTH_PASSDSS_STRING, HEX_STR_KEY_VERIF_ERROR);
	    ret = -1;
	    goto error;
	  }
	}
	...
	if ((keysize = DH_compute_key((unsigned char*)s, server_pubkey, dh)) < 0) {
		Display(LOG_LEVEL_3, ELError, TSP_AUTH_PASSDSS_STRING,
			HEX_STR_DH_SHARED_COMPUTE_ERROR);
		ret = -1;
		goto error;
	}
	...
	Display(LOG_LEVEL_3, ELDebug, TSP_AUTH_PASSDSS_STRING,
		HEX_STR_DH_SHARED_KEY, BN_bn2hex(dh_K));
	...
	/* Debug information available at level 4 */
 {
	 ...
	 Display(LOG_LEVEL_3, ELDebug, TSP_AUTH_PASSDSS_STRING,
		 HEX_STR_SIGNED_HASH, BN_bn2hex(h));
	 BN_free(h);
 }
	 Display(LOG_LEVEL_3, ELDebug, TSP_AUTH_PASSDSS_STRING,
		 HEX_STR_DSA_SIGN_R, BN_bn2hex(sig->r));
	 Display(LOG_LEVEL_3, ELDebug, TSP_AUTH_PASSDSS_STRING,
		 HEX_STR_DSA_SIGN_S, BN_bn2hex(sig->s));
	 
	/* Verify that the DSS signature is a signature of hash */
	ret = DSA_do_verify(hash, sizeof(hash), sig, dsa) ;
	switch (ret) {
	case 0:
		Display(LOG_LEVEL_3, ELError, TSP_AUTH_PASSDSS_STRING,
			HEX_STR_BAD_SIG_FROM_SERVER);
		ret = -1;
		goto error;
		break; /* NOTREACHED */
	case 1:
	...
	memcpy(enc_key, tmphash, 24);
 {
	 ...
	 Display(LOG_LEVEL_3, ELDebug, TSP_AUTH_PASSDSS_STRING,
		 HEX_STR_PASSDS_ENC_KEY, BN_bn2hex(enc));
	 Display(LOG_LEVEL_3, ELDebug, TSP_AUTH_PASSDSS_STRING,
		 HEX_STR_PASSDS_IV, BN_bn2hex(iv));
	 Display(LOG_LEVEL_3, ELDebug, TSP_AUTH_PASSDSS_STRING,
		 HEX_STR_PASSDS_INTEG_KEY, BN_bn2hex(i));
	 BN_free(enc);
	 BN_free(i);
	 BN_free(iv);
 }
	...
	*/
	...
	if (tspIsRedirectStatus(tspGetStatusCode(BufferIn))) {
		if (tspHandleRedirect(BufferIn, conf, broker_list) == TSP_REDIRECT_OK) {
			ret = BROKER_REDIRECTION;
		}
		else {
			ret = BROKER_REDIRECTION_ERROR;
		}

		goto error;
	}

	/* Read response. Expect success (200), else display error */
	if (memcmp(BufferIn, "200", 3)) { /* Not successful */
		Display(LOG_LEVEL_3, ELError, TSP_AUTH_PASSDSS_STRING, HEX_STR_NO_SUCCESS, BufferIn+4);
		ret = -1;
	} else {
		ret = 0;
	}
