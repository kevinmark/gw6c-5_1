...
----------------------------------------------------------------------
 $Id: dns.c,v 1.4 2007/05/23 19:19:34 cnepveu Exp $
----------------------------------------------------------------------
...
DNSGenerateID(void)
{
	...
	if (!id)
		id = (uint16_t) (0xffff & (now.tv_sec ^ now.tv_usec ^ getpid()));
	else
 ...
 */
static tDNSRCode
DNSUpdate(SOCKET Socket, char *Name, char *Domain, char *AAAA)
{
	uint16_t DNSid;
	uint16_t DNSidReply;
	uint16_t Header;
	...
	if (NetTCP6Write(Socket, Buffer, (int)Len) != (int)Len)
 ...
 */
SOCKET
DNSUpdateConnect(char *Server)
{
	return NetTCP6Connect(Server, DNS_UPDATE_PORT);
}
 ...
 */
tDNSRCode
DNSUpdateAddAAAA(SOCKET Socket, char *Name, char *Domain, char *AAAA)
{
 ...
 */ 
tDNSRCode
DNSUpdateDelRRSets(SOCKET Socket, char *Name, char *Domain)
{
