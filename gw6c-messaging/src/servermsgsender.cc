// **************************************************************************
// $Id: servermsgsender.cc,v 1.14 2007/06/22 18:34:43 cnepveu Exp $
//
...
void ServerMsgSender::Send_TunnelInfo( const Gw6cTunnelInfo* aTunnelInfo )
{
  ...
  if( aTunnelInfo->szBrokerName ) {
    ...
    nDataLen += (WORD)strlen(aTunnelInfo->szBrokerName) + 1;
  }
  else {
  ...
  if( aTunnelInfo->szIPV4AddrLocalEndpoint ) {
    ...
    nDataLen += (WORD)strlen(aTunnelInfo->szIPV4AddrLocalEndpoint) + 1;
  }
  else {
    memset( pData + nDataLen, 0x00, 1 );
    ++nDataLen;
  ...
  if( aTunnelInfo->szIPV6AddrLocalEndpoint ) {
    ...
    nDataLen += (WORD)strlen(aTunnelInfo->szIPV6AddrLocalEndpoint) + 1;
  }
  else {
    memset( pData + nDataLen, 0x00, 1 );
    ++nDataLen;
  ...
  if( aTunnelInfo->szIPV4AddrRemoteEndpoint ) {
    ...
    nDataLen += (WORD)strlen(aTunnelInfo->szIPV4AddrRemoteEndpoint) + 1;
  }
  else {
    memset( pData + nDataLen, 0x00, 1 );
    ++nDataLen;
  ...
  if( aTunnelInfo->szIPV6AddrRemoteEndpoint ) {
    ...
    nDataLen += (WORD)strlen(aTunnelInfo->szIPV6AddrRemoteEndpoint) + 1;
  }
  else {
    memset( pData + nDataLen, 0x00, 1 );
    ++nDataLen;
  ...
  if( aTunnelInfo->szDelegatedPrefix ) {
    ...
    nDataLen += (WORD)strlen(aTunnelInfo->szDelegatedPrefix) + 1;
  }
  else {
    memset( pData + nDataLen, 0x00, 1 );
    ++nDataLen;
  ...
  if( aTunnelInfo->szUserDomain ) {
    ...
    nDataLen += (WORD)strlen(aTunnelInfo->szUserDomain) + 1;
  }
  else {
    memset( pData + nDataLen, 0x00, 1 );
    ++nDataLen;
...
void ServerMsgSender::Send_BrokerList( const Gw6cBrokerList* aBrokerList )
{
  ...
  do
  {
    ...
    if( list->szAddress ) {
      ...
      nDataLen += (WORD)strlen(list->szAddress) + 1;
    }
    else {
      memset( pData + nDataLen, 0x00, 1 );
      ++nDataLen;
  ...
  if( list != NULL )
  {
    do
    {
      ...
      if( list->device_name ) {
        ...
        nDataLen += (WORD)strlen(list->device_name) + 1;
      }
      else {
        memset( pData + nDataLen, 0x00, 1 );
        ++nDataLen;
