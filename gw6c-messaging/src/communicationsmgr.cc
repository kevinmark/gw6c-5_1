// **************************************************************************
// $Id: communicationsmgr.cc,v 1.10 2007/01/30 18:53:30 cnepveu Exp $
//
...
// --------------------------------------------
#define COMMMGR_PIPE_NAME   "\\\\.\\pipe\\gw6cmessaging-ipc"
...
//
// Return values:
//   GW6CM_UIS__NOERROR: On successful initialization.
...
void CommunicationsManager::Work( void )
{
  ...
  while( !ShouldStop()  &&  GetState() != STATE_FATALERROR )
  {
    ...
    if( GetState() == STATE_DISCONNECTED )
    {
      ...
      if( (retCode = Initialize()) == GW6CM_UIS__NOERROR )
      {
        // Reset error count.
        nErrorCount = 0;
...
void CommunicationsManager::GetStatistics( MANAGER_STATISTICS* aStats )
{
  ...
  aStats->nMsgQueued    = m_SendQueue.size();
...
void SenderThread::Work( void )
{
  ...
  while( !ShouldStop() )
  {
    ...
    // ---------------------------------------------------------------
    if( (retCode = m_CommMgr->m_Servent.CanRead( bCanRead )) != GW6CM_UIS__NOERROR )
    {
      // An error occured.
    ...
    if( bCanRead )
    {
      ...
      SLEEP( 20 );    // Wait for message to be read by the receiver thread.
      continue;
    }
...
void ReceiverThread::Work(void)
{
  ...
  while( !ShouldStop() )
  {
    ...
    if( !bCanRead )
    {
      SLEEP( 25 );     // Wait for messages to be available.
      continue;
    }
