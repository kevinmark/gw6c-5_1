// **************************************************************************
// $Id: namevalueparser.cc,v 1.14 2007/10/11 19:41:00 cnepveu Exp $
//
...
// **************************************************************************
#include <gw6cconfig/namevalueparser.h>
...
bool NameValueParser::ReadConfigurationData( const string& aFilename )
{
  ...
  while( !inStream.eof() )
  {
    ...
    else
    {
      ...
      throw GW6C_UIS__NMP_BADCONFIGFILE;
    }
  }
...
bool NameValueParser::WriteConfigurationData( const string& aFilename )
{
  ...
  {
		// Open failed.
		throw GW6C_UIS__NMP_OPENFAILWRITE;
  }
      ...
      switch( iter->first )
      {
        ...
        case NVP_DATA_COMMENT:
          if( strncmp( iter->second.c_str(), NV_COMMENT_GENERATE, strlen(NV_COMMENT_GENERATE) ) == 0 )
            outStream << NV_COMMENT_GENERATE << asctime( timeinfo );
          else
